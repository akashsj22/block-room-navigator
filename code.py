...
def build_building_graph():
    ...
    connections = [
        ...
        # ── STAIRS: per-floor nodes chained F1→F2→F3→F4→F5 ─────────────
        ...
        # ── LIFT: per-floor nodes chained F1→F2→F3→F4→F5 ─────────────
        ...
        # ── FLOOR 5 stairs/lift connections ──────────────────────────
        ...
        (605, 'Stairs_F6', 2),
        (621, 'Stairs_F6', 2),
        (641, 'Stairs_F6', 2),   # assume F6 touches F7 stairs somewhere
    ...
    while True:
        ...
        if user_choice == '1':
            ...
            if path:
                ...
                # Collapse consecutive transit nodes into single actions
                display_steps = []
                i = 0
                while i < len(path):
                    node = path[i]
                    if isinstance(node, str) and node.startswith('Lift_F'):
                        # Collect all consecutive Lift nodes
                        lift_nodes = []
